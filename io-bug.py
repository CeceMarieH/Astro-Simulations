...
while True: #loop through calcs to animate
    ...
    U = 0.5*io.mass*dot(io.vel,io.vel) - G*jup.mass*io.mass/mag(io.pos - jup.pos) - G*eu.mass*io.mass/mag(io.pos - eu.pos) - gan.mass*io.mass/mag(io.pos - gan.pos)
    
    #period (using Kepler's Third Law)
    per = sqrt(4*pi**2*sma**3/(G*(io.mass+jup.mass)))/DAY
    pertest = sqrt(2*pi*sma/G/(io.mass + jup.mass))
    
    #eccentricity
    ecc = (rmax - rmin)/(rmax + rmin)
    #ecc = sqrt(1.0 + (2*dot(l,l)*E))#/((G*jup.mass)**2*(io.mass)**3))
    ...
    #tf = 3*G*jup.mass*io.mass*Rio/(4*r**3)
    

    
    #rotation of io
    omega = mag(io.vel)*h/sma
